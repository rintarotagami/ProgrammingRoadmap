  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-between p-24 pt-8">
      <div className="w-full">
        <ul className="flex flex-row justify-start items-center overflow-x-auto">
          {Array.from({ length: 10 }).map((_, index) => (
            <li key={index} className="mr-4">
              <img src="sample\dummy-icon.jpg" alt={`フォロー${index + 1}`} className="w-12 h-12 rounded-full" />
              <p className="text-center text-sm">RinIogi</p>
            </li>
          ))}
        </ul>
        <ul className="flex flex-wrap justify-start mt-4">
